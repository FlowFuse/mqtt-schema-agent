...
class Agent {
    ...
    async connect () {
        ...
        try {
            ...
            this.client.on('message', function (topic, payload) {
                ...
                const existing = agent.topics[topic]
                if (existing) {
                    if (JSON.stringify(existing.type) !== JSON.stringify(type)) {
                        // changed update
                        agent.topics[topic] = {
                            timestamp: Date.now(),
                            type
    ...
    async upload () {
        ...
        Object.keys(this.topics).forEach((key) => {
            if (this.topics[key].timestamp > this.lastUpload) {
                upload.push({
                    topic: key,
        ...
        if (upload.length > 0) {
            try {
                ...
                this.lastUpload = Date.now()
                logger.log(`uploaded ${upload.length}`)
    ...
    async primeCache () {
        try {
            const topics = await got.get(`${this.options.forgeURL}/api/v1/teams/${this.options.team}/brokers/${this.options.broker}/topics`, {
                headers: {
                    Authorization: `Bearer ${this.options.token}`
                }
            }).json()
