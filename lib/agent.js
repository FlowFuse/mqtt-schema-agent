...
class Agent {
    ...
    async connect () {
        ...
        this.reportInterval = setInterval(async () => {
            if (agent.connected) {
                console.log(`${agent.options.forgeURL}/api/v1/teams/${agent.options.team}/broker/${agent.options.broker}/topics`)
                try {
                    await got.post(`${agent.options.forgeURL}/api/v1/teams/${agent.options.team}/broker/${agent.options.broker}/topics`, {
                        headers: {
                            Authorization: `Bearer ${this.options.token}`
                        },
                        json: agent.topics
